...
6. Enter a password
7. Set Display Name - this will be the person's name or nickname if they prefer. This is what other users see when discovering friends or on posts
...
1. Add SMTP server settings for your email provider (e.g., Gmail)
...
5. **Option A:** Generate a new pairing token, pass to the admin of the node you wish to connect to
   - Once they initiate connection - if it works you'll see their node appear at the bottom of the page in Full Node Connections
6. **Option B:** Add in the hostname and pairing code given to you by the admin of the other node.
   - Initiate connection, if successful you should see their node appear at the bottom of the page in Full Node Connections
7. Give the connected node a nickname so you know who it belongs to. (e.g., Bob's Home Node) All your users will see any users, pages and groups from that node with that nickname

**Note:** Targeted subscriptions are created automatically when joining/following groups, events and pages that have been discovered by the node via other nodes that are not directly connected to yours ('word of mouth' sharing). They are only for this function and not for users. An upgrade to a Full Node Connection is the only way to find the users on that node, plus all other groups and pages they may have on there.

## 12. Make a Local Post as Admin
...
## 13. Configure Your Reverse Proxy

1. Out of scope for this document but recommend an NGINX docker container or for an easier GUI based setup - NGINX PROXY MANAGER docker container
2. This will also require either a permanent IP address from your ISP and domain name from somewhere (expensive) or use a (usually free) DDNS service to keep a DDNS domain name up to date as your IP changes. Dynu IP Update Client and Dynu.com for the domain name is one example.

## 14. Let Your Users Loose!
...
3. Enter the group's name - this is what will be shown on Discover Groups
4. Write a short description
5. Assign a local user as the initial group admin. You can assign more than one and once up and running group members on federated nodes can be made additional group admins/mods.
6. This initial admin(s) cannot be demoted or removed by other admins. And if that does somehow happen, the node admin can reassign them here.

### Create a Public Page

1. Click on 'Manage Public Pages' in the Admin Dashboard
2. Click on 'Add New Public Page'
3. As with a normal user add a Username (email address)
4. Create a password
5. Create the Display Name - this is the name people see. (e.g., Bob's Window Cleaning Service)
6. Managing this page involves logging in like a normal user. To all intents and purposes it works more or less like a normal user profile, but with slight differences like posting, followers etc.
